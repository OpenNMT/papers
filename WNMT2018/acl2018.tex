...
\caption{\small Pareto on accuracy and throughput.}
\label{fig:pareto}
\end{figure}
...
speed-up of \textcolor{red}{x18} on
WNMT 2018 English-German newstest2014.
Our second system {\tt distill-tiny-opt} achieves 23.11 on the same testset (-4.85 behind the reference
model) but with an additional decoding speed-up of \textcolor{red}{x6}.
The final model reaches 1000 words/sec on the evaluation CPU hardware and is the fastest CPU model submitted. \textcolor{red}{To our knowledge, the current fastest system reported on CPU is only 100 words/sec \cite{DBLP:journals/corr/Devlin17}.}
...
% \textcolor{red}{x6} at about 1000 words/sec on the evaluation
% hardware.
...
Table \ref{table:config} with their evaluation in Table \ref{table:eval}. For both, the
...
\caption{\small Profiling of the throughput during inference on newstest2014.
%Each line is a different system.
The {\tt small-1,2,5 (LSTM)} are distilled systems with respective beam size of 1, 2, 5. {\tt tiny-opt-2 (GRU)} and {\tt small-opt-2 (LSTM)} are the final systems. The total decoding time for each of these systems (in seconds) are from top to bottom: 60, 464, 754, 662 and 629. The top table show the distribution of time per component of the network, while the bottom table show the distribution of time for network operators.}
...
and softmax require significant time. This time is scaling linearly
...
alignments. For each source phrase $f_1 \ldots f_n$, we consider the
...
extends vocabulary mapping to multi-word expressions\footnote{For
  ...
  translated by {\tt \`{A}  nouveau} will need the 2 additional meanings {\tt \`{A} } and {\tt nouveau} not
...
\begin{tabular}{cll}
\hline
\multirow{2}{*}{Transformer (Tf.)}   & Base          & N=6, d=512, \(\displaystyle d_{ff}\)=2048, h=8                    \\ \cline{2-3}
                               & Large         & N=6, d=512, \(\displaystyle d_{ff}\)=4096, h=8                    \\ \hline
...
\end{tabular}
\caption{\small Configurations for the NMT systems}
\label{table:config}
\end{table*}
...
\hline
\multicolumn{2}{c}{\multirow{2}{*}{}}    & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}quantize\\ runtime\end{tabular}} & \multirow{2}{*}{vmap} & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}quantize\\ model\end{tabular}} & \multicolumn{2}{c}{newstest2014} & \multicolumn{2}{c}{newstest2015} & \multirow{2}{*}{model size} \\ \cline{6-9}
\multicolumn{2}{c}{}                     &                                                                             &                       &                                                                           & cpu time {[}s{]}     & BLEU      & cpu time {[}s{]}     & BLEU      &                             \\ \hline
\multirow{2}{*}{Tf.} & Base      & -                                                                           & -                     & -                                                                         & 10795.16
                     & 27.30     &          -            & 29.36     &    1.2G                         \\
                             & Large     & -                                                                           & -                     & -                                                                         &   11279.97
                   & 27.96     &           -           & 29.95     &    1.4G                         \\ \hline
\multirow{2}{*}{direct}      & Large     & -                                                                           & -                     & -                                                                         &                      & 24.56     &                      & 27.24     & 618M                        \\
                             & Small     & -                                                                           & -                     & -                                                                         &                      & 23.78     &                      & 26.37     & 416M                        \\ \hline
\multirow{6}{*}{distilled}   & Small     & -                                                                           & -                     & -                                                                         & 1694.16              & 25.96     & 1300.24              & 28.62     & 416M                        \\
...
lists the different configurations for these systems:

\begin{itemize}
\item {\tt distill-small-opt}, uses a bidirectional RNN with 2 LSTM layers with each hidden layer having 1024 nodes.
...
\item {\tt distill-tiny-opt}, uses  a smaller network, with GRU layers, 512 hidden size.
...
Table \ref{table:eval} shows our internal evaluations.
...
We also compare the influence of quantization hyperparameters, e.g. vmap and quantize model, on system {\tt distill-tiny-opt}.
...
of quantization, the speed measurement is very dependent on other
...
model for CPU demonstrating again the powerful effect of the distillation process and for the first time cross-class. Most of the additional presented optimizations can apply for other architecture. Our final system does show a significant increase in speed of \textcolor{red}{x113} over the baseline and demonstrate a throughput of 1000 word/s on a single core which is the fastest reported so far...
